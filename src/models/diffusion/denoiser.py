...
class Denoiser(nn.Module):
    def __init__(self, cfg: DenoiserConfig) -> None:
        super().__init__()
        self.cfg = cfg
        self.inner_model = InnerModel(cfg.inner_model, noise_conditioning=cfg.noise_conditioning) # InnerModel(cfg.inner_model)
    ...
    #     c_noise = sigma.log() / 4
    #     return Conditioners(*(add_dims(c, n) for c, n in zip((c_in, c_out,
    #     c_skip, c_noise), (4, 4, 4, 1, 1))))
    ...
    def compute_conditioners(self, sigma: Tensor) -> Conditioners:
        ...
        if self.cfg.noise_conditioning:              # baseline DIAMOND
            c_out = sigma * c_skip.sqrt()
            c_noise = sigma.log() / 4
        else:                                        # uDIAMOND
            c_out   = torch.ones_like(sigma)         # ← uEDM rule
            c_noise = torch.zeros_like(sigma)        # embed will be all‑zeros

        return Conditioners(
            *(
                add_dims(c, n) for c, n in zip((c_in, c_out, c_skip, c_noise), (4, 4, 4, 1))
            )
        )
